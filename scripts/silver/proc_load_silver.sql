...
BEGIN 
	...
	BEGIN TRY 
		...
		SELECT 
			cst_id,
			cst_key,
			TRIM(cst_firstname) AS cst_firstname, --Removed extra spaces
			TRIM(cst_lastname) AS cst_lastname, --Removed extra spaces
			CASE 
				WHEN TRIM(UPPER(cst_material_status)) = 'S' THEN 'Single'
				WHEN TRIM(UPPER(cst_material_status)) = 'M' THEN 'Married'
				ELSE 'n/a'
			END AS cst_martial_status, --Data Normalization or standardization and handling missing values
			CASE 
				WHEN TRIM(UPPER(cst_gndr)) = 'M' THEN 'Male'
				WHEN TRIM(UPPER(cst_gndr)) = 'F' THEN 'Female'
				ELSE 'n/a'
			END AS cst_gndr, --Data Normalization or standardization and handling missing values
			cst_create_date
		FROM (
		SELECT *,
		ROW_NUMBER() OVER(PARTITION BY cst_id ORDER BY cst_create_date DESC) AS flag_ind
		FROM bronze.crm_cust_info WHERE cst_id IS NOT NULL)t --Remove duplicates
		WHERE flag_ind = 1;
